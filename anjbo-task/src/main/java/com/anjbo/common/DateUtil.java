...
public class DateUtil {
	...
	public synchronized static String getDateString() throws InterruptedException{
	 ...
	 /**
		 * 传入long类型时间，返回字符串格式日期
		 * @param longTime 传入Long类型对应的字符串时间
		 * @param fmt_type 返回时间格式类型
		 * @return
		 */
	 public static String longToDate(String longTime,String fmt_type) {		 
  		BigDecimal bd = new BigDecimal(longTime); 
  		bd = bd.setScale(0,BigDecimal.ROUND_DOWN);
  		long lSysTime1=Long.parseLong(bd.toPlainString());
		SimpleDateFormat sdf= new SimpleDateFormat(fmt_type);
