...
public class DingtalkServiceImpl implements DingtalkService {
	...
	static {
        try {
        	dingUrl = ThirdApiConstants.DINGTALK_WS_URL;
        } catch (Exception e) {
	...
	public DingtalkUser getUserByName(String userName) {
		try{
			...
			String url = String.format("%s/%s", dingUrl, Enums.DingtalkServiceMethod.USER_INFO.getValue());
			String result = SingleUtils.getRestTemplate().postForObject(url,SingleUtils.getHttpEntity(map,null),String.class);
	...
	public String bpmsCreate(ThirdDingtalkBpmsDto dto) {
		try{
			...
			String url = String.format("%s/%s", dingUrl, Enums.DingtalkServiceMethod.SEND_PROCESS_INSTANCE.getValue());
			String result = SingleUtils.getRestTemplate().postForObject(url,SingleUtils.getHttpEntity(map,null),String.class);
			JSONObject json = JSONObject.parseObject(result);
	...
	public List<DeptDto> getDeptList() {
		...
		try{	
			...
			String url = String.format("%s/%s", dingUrl, Enums.DingtalkServiceMethod.DEPT_LIST.getValue());
			String result = SingleUtils.getRestTemplate().postForObject(url,SingleUtils.getHttpEntity(map,null),String.class);
	...
	public List<UserDto> getUserList() {
		...
		try{			
			...
			String url = String.format("%s/%s", dingUrl, Enums.DingtalkServiceMethod.USER_LIST.getValue());
			String result = SingleUtils.getRestTemplate().postForObject(url,SingleUtils.getHttpEntity(map,null),String.class);
			JSONArray jsonAry = JSONArray.parseArray(result);
