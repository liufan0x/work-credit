angular.module("anjboApp").controller("lendingInstructionsEditCtrl",function($scope,$http,$state,box,route){
	...
	$http({
		...
		$http({
			...
			data:{"cooperativeAgencyId":$scope.order.cooperativeAgencyId,'productId':$scope.productId}
		}).success(function(data){
...
//			 $scope.order.chargeMoney=$scope.order;
	...
	function findFund(){
		...
		if(null!=productId&&productId>0&&null!=cooperativeAgencyId&&cooperativeAgencyId>0&&borrowingDays>0&&riskGradeId>0&&loanAmount>0){
			$http({
				method: 'POST',
				data:{"cooperativeAgencyId":cooperativeAgencyId,'productId':productId,'borrowingDays':borrowingDays,"riskGradeId":riskGradeId,"loanAmount":loanAmount},
