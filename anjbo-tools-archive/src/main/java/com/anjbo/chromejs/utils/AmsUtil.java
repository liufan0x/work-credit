...
public class AmsUtil {
	...
	public static RespStatus smsSend(String phone, String ip, String smsContent,
		...
		try {
			...
			String url = Constants.AMS_URL+SMSURL2+"?"+getHttpUrl(params);
			...
			Map<String, String> map = parseAMSResult(result);
	...
	public static RespStatus emailSend(String title, String email, String content) throws MortgageException{
		...
		try {
			...
			params.put("flag", "");
			String result = StringUtils.EMPTY;
			String url = Constants.AMS_URL+EMAILSEND+"?"+getHttpUrl(params);
			result = HttpUtil.get(url);
			if (result == null) {
				return new RespStatus(RespStatusEnum.REQUEST_TIMEOUT.getCode(),
						RespStatusEnum.REQUEST_TIMEOUT.getMsg());
	...
	public static String getHttpUrl(Map<String, String> params){
		...
		for (Entry<String, String> entry : params.entrySet()) {
			...
			builder.append("=");
			builder.append(StringUtils.trimToEmpty(entry.getValue()));
			if (i!=size){
				builder.append("&");
			} 
			i++;
	 ...
	 * @return
	 */
	public static Map<String,String> parseAMSResult(String urlparam){
