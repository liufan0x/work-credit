...
public class RedisOperator {
    ...
    public static void initialPool(JedisPoolConfig config, String host, int port, String pwd) {
        if (jedisPool == null) {            
    ...
    public static String showS(String key,String value) {
    	Jedis jedis = getConnection();                //打开链接
		if (jedis == null) {
			return null;
		}
    	try {
    		String conn = jedis.set(key, value);
    		returnConnection(jedis);                   //关闭redis连接
    		return conn;
		} catch (Exception e) {
    ...
    public static String showG(String key) {
    	Jedis jedis = getConnection();
    	if (jedis==null) {
			return null;
		}
    	try {
		String conn =jedis.get(key);
		returnConnection(jedis);
		return conn;
		} catch (Exception e) {
			// TODO: handle exception
