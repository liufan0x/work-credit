...
public class CookieUtils {
	 ...
	 * 验证码验证
	 * 
	 * @return Boolean true为匹配失败,false成功
	 */
	public static boolean authCode(HttpSession session, String authCode) {		
		return authCode(session, authCode, false);
	}
	public static boolean authCode(HttpSession session, String authCode, boolean isSmsCode) {
		String codeKey = isSmsCode ? com.anjbo.common.Constants.LOGIN_USER_CODE_SMS : Constants.KAPTCHA_SESSION_KEY;
		String code = String.valueOf(session.getAttribute(codeKey));
		log.debug("服务器端的验证码是=" + code);
		// 校验后移除session
