...
var approvalSeal = function () {
	//类构造方法
	function approvalSeal() {
		_classCallCheck(this, approvalSeal);
	...
	_createClass(approvalSeal, [{
		...
		value: function KVCFunc(data) {
			...
			if (data.length) {
				for (var i = 0; i < data.length; i++) {
					var applierName = data[i].applierName;
					var obj = {
						id: data[i].id,
						...
						title: data[i].title,
						headName: applierName ? applierName.substr(applierName.length - 2, 2) : "",
						colorClz: applierName ? "img_bg_color_" + applierName.charCodeAt(0) % 3 : "img_bg_color_1"
						//状态(0待审批,1审批通过,2审批拒绝)
					};switch (data[i].state) {
						case 0:
							obj.stateStr = "待审批";
		...
		value: function clickFunc() {
			...
			$("div.contentPart").on("click", "div.cell", function () {
				var id = $(this).data("id");
				window.location.href = "/elementH5/template/approval_seal_detail.html?id=" + id + "&userid=" + self.userid;
			});
		}
	},{//下拉刷新与上拉加载
		key: "initFresh",
		value: function initFresh() {
			...
			window.onscroll = function () {
				if (screenHight + $(document).scrollTop() >= $('div#approSealController').height() + 60) {
					if (self.load) {
						//缓冲加载菊花
						loading();
						setTimeout(function () {
