angular.module("anjboApp", ['bsTable']).controller("monitorArchiveListCtrl",function($scope,$filter,$compile,$http,$state,box,process,parent){
    ...
    window.operateEvents = {
        'click .closeOrder': function (e, value, row, index) {
            var closeOrder = function(){
                $http({
                    method: 'POST',
                    url:'/credit/tools/monitorArchive/v/delete',
                    data:{"id":row.id}
                }).success(function(data){
        ...
        'click .lookOrder': function (e, value, row, index) {
            $state.go("orderDetail",{'orderNo':row.orderNo, 'processId':row.processId, 'productCode':'01', 'relationOrderNo':'0'});
        },
    ...
    $scope.query = function(){
        $("#table").bootstrapTable('refresh', {url: '/credit/tools/monitorArchive/v/page'}); 
    }
    ...
    $scope.archiveList = {
            options: {
                method:"post",
                url:"/credit/tools/monitorArchive/v/page",
                ...
                columns: [{
                    ...
                    formatter:function(value,row,index){
                    	...
                    	return $filter('date')(row.startTime, 'yyyy-MM-dd') + "至" + $filter('date')(row.endTime, 'yyyy-MM-dd');
                    }
                }, {
                    title: '查询频率',
