...
(function($) {
	...
	$.fn.extend({
		...
		photoGallery: function(options) {
			if(options.funStr == "undefined") {
				var ifDel = "none";
			} else {

				var ifDel = "inline-block";
			}
			...
			var defaults = {
				...
				template: {
					...
					OPERTATION: '<div class="oper">' +
						...
						'<span class="oper_rotate_right" title="向右旋转"><i class="icon_tool-rotate_r"></i></span>' +
						'<span class="oper_rotate_left" title="向左旋转"><i class="icon_tool-rotate_l"></i></span>'+
						'<span class="oper_download" title="删除" style="display:' + ifDel + '"><i class="icon_tool-delete"></i></span>' +
						'</div>' +
						'</div>',
					//缩略图
			...
			$download.on("click", function() {
				...
				if(options.funStr == "undefined") {
					...
					var ss = "parent." + options.funStr + "('" + imgUrl + "')";
					eval(ss);
				}
				$(".image.active").remove();
				//$(".image:eq("+o.activeIndex+")").remove();
				$(".image").each(function(i) {
					$(this).attr("index", i);
				})
				o.imgs.del(o.activeIndex);
				if(o.imgs.length == 0) {
					$(".closeWin").trigger("click");
				} else {
					if(o.activeIndex < o.imgs.length - 1) o.activeIndex++;
					else if(o.activeIndex > 0) o.activeIndex--;
				}
			...
			$rotate_left.on("click", function() {
				...
				if(rotateClass) {
					var nextDeg = (rotateClass[2] * 1 -90 + 360) % 360;
					$image.removeClass(rotateClass[0]).addClass("rotate" + nextDeg);
					$thumbImg.removeClass(rotateClass[0]).addClass("rotate" + nextDeg);
					resizeImage(nextDeg);
					resizeThumbImg(nextDeg);
	...
	$.extend({
		//打开图片查看器
		openPhotoGallery: function(obj, funStr) {
			localStorage["funStr"] = funStr;
			...
			var img = $img[0],
				...
				maxHeight = $(window).height() - windowMargin * 2,
...
//			$.getScript("/plugins/jquery-photo-gallery/dragiframe.js");
		},
		//做初始化
		initGallery: function() {
			...
			localStorage.removeItem("funStr");
			...
			//拖动缩放ifame
			var flag = false
			$('.scaleBtn').mousedown(function(e){
				flag = true
				console.log(flag)
			})
			$('.scaleBtn').mouseup(function(e){
				flag = false
				console.log(flag)
			})
			$(document).mousemove(function(e){
				if(flag){
					// $('img.active').css({
					// 	width:`${e.clientX+10}px`,
					// 	height:`${e.clientY+10}px`
					// })
					$(_jg).css({
						'width':`${e.clientX+10}px`,
						'height' : `${e.clientY+10}px`
			...
			$(document).bind('mousewheel',function(){
				...
				// 禁止滚动时改变ifame的尺寸
				$(_jg).css({
					'width':$('img.active').width() + 'px',
					'height' : $('img.active').height() + 'px',
				});
			});
		}
	});
