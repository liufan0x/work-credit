 ...
 * 
 */
public class SameBxGjjLoanServiceImpl extends LoanService {
	private static final Log log = LogFactory
			.getLog(SameBxGjjLoanServiceImpl.class);
	...
	public SameBxGjjLoanServiceImpl(LoanConditionDto loanConditionDto) {
		this.loanConditionDto = loanConditionDto;
		monthlyRates = loanConditionDto.getGjjRates() / 100 / 12;
	...
	public String calcTotalRefund() {
		...
		// 总还款金额
		String totalRefund = NumberUtil.formatDecimalToStr(gjjTotal
				+ totalInterest);
		return totalRefund;
	}
	...
	private double getMonthlyPayments() {
		...
		if (monthlyPayments == null) {
			monthlyPayments = (gjjTotal * monthlyRates * Math.pow(
					(1 + monthlyRates), months))
					/ (Math.pow(1 + monthlyRates, months) - 1);
			monthlyPayments = NumberUtil.formatDecimal(monthlyPayments);
	...
	private double getTotalInterest() {
		...
		if (totalInterest == null) {
			...
			totalInterest = months * monthlyPayments - gjjTotal;
	...
	private double getMonthlyPrincipal(int index) {
		...
		double monthlyPrincipal = gjjTotal * monthlyRates
	...
	private double getMonthlyInterest(int index) {
		double monthlyInterest = gjjTotal
				* monthlyRates
