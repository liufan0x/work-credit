 ...
 * @date 2016-6-2 下午07:19:35
 */
public class IDCardCheckerUtil {
	/**
	 * 完整的身份证号码
	 */
	 ...
	 * @param cardNumber
	 */
	public IDCardCheckerUtil(String cardNumber) {
		if (null != cardNumber) {
	...
	public String populationType() {
		...
		calendar.setTime(this.cacheBirthDate);

		int year = now.get(Calendar.YEAR) - calendar.get(Calendar.YEAR);

		if (year < 2)
			type = "婴儿";
		else if ((year >= 2 && year <= 12))
			type = "儿童";
		else
			type = null;
	...
	private static String contertToNewCardNumber(String oldCardNumber) {
		...
		buf.append(IDCardCheckerUtil.calculateVerifyCode(buf));
		return buf.toString();
	}
